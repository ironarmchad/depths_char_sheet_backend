...
def test_character_new_post(app, client, auth, populate_db):
...
def test_character_get(client, auth, populate_db):
    auth.login('test1', 'test')
...
def test_character_get_wrong_owner(client, auth, populate_db):
...
def test_character_get_no_character(app, client, auth):
...
def test_character_patch(app, client, auth, populate_db):
    with app.app_context():
...
def test_character_delete(app, client, auth, populate_db):
    with app.app_context():
...
def test_character_delete_bad_request(client, auth, populate_db):
    auth.login('test2', 'test')
